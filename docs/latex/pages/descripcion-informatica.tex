...
\begin{itemize}
    ...
    \item \textbf{Arquitectura y Análisis (Sección \ref{sec:arquitectura}):} Se describirá la arquitectura de alto nivel de la aplicación, ilustrando la interacción entre sus componentes principales (\textit{frontend}, \textit{backend}, base de datos) mediante diagramas.
    ...
    \item \textbf{Distribución y Despliegue (Sección \ref{sec:distribucionDespliegueCap4}):} Se explicará cómo se empaqueta la aplicación para su distribución y los pasos necesarios para su ejecución en un entorno basado en contenedores.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item Acceder a un panel de control (\textit{dashboard}) con estadísticas sobre el uso de la aplicación (crecimiento de usuarios, número de objetos y comentarios, etc).
...
Para una comprensión completa de las funcionalidades de LogArt, es esencial definir los conceptos o entidades fundamentales que estructuran la información dentro de la aplicación. Estos conceptos, son los siguientes:
...
\begin{itemize}
    \item \textbf{RF6.1: Acceso al Panel de Control (\textit{Dashboard}).} El administrador tendrá acceso a una sección administrativa (\textit{dashboard}) no visible para otros usuarios.
    \item \textbf{RF6.2: Visualización de Estadísticas de la Aplicación.} El \textit{dashboard} permitirá al administrador visualizar gráficos y datos sobre el desempeño de la aplicación, como:
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{RF6.3: Moderación de Objetos.} El administrador podrá visualizar todas las entradas creadas en la plataforma (independientemente del estado de compartición) y eliminar aquellas que considere inapropiadas o que infrinjan normativas.
...
\begin{itemize}
    ...
    \item \textbf{RF7.4: Manejo de Errores y Retroalimentación.} El sistema proporcionará mensajes de error claros y útiles en caso de fallos o entradas incorrectas. Se mostrarán páginas de error apropiadas para URLs no encontradas o errores del servidor.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item El \textit{frontend} se optimizará (ej. mediante el proceso de build de Vite) para asegurar una carga inicial rápida y una experiencia fluida.
    \end{itemize}
    \vspace{0.3cm}
    ...
    \begin{itemize}
        ...
        \item Se implementará un manejo de errores adecuado tanto en el \textit{backend} como en el \textit{frontend} para gestionar situaciones anómalas de forma controlada.
        \item La contenedorización con Docker contribuirá a la fiabilidad del sistema al asegurar entornos consistentes.
    \end{itemize}
    \vspace{0.3cm}
    \item \textbf{RNF5: Mantenibilidad y Extensibilidad.}
    \begin{itemize}
        \item El código fuente, tanto del \textit{backend} como del \textit{frontend}, se organizará de forma modular y siguiendo principios de diseño que faciliten su comprensión, modificación y futura extensión.
        \item Se utilizarán nombres descriptivos para variables, funciones, clases, módulos y componentes.
    ...
    \begin{itemize}
        \item La aplicación \textit{frontend} deberá ser compatible y funcionar correctamente en las versiones más recientes de los principales navegadores web modernos.
    \end{itemize}
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Capa de Presentación (Cliente - \textit{Frontend}):}
    ...
    \begin{itemize}
        ...
        \item Comunicarse con la capa de aplicación (\textit{backend}) a través de peticiones HTTP a su API RESTful para enviar y recibir datos.
        \item Gestionar el estado de la interfaz y la navegación dentro de la SPA sin recargas completas de página
    \end{itemize}
    La estructura detallada de los componentes del \textit{frontend} se describirá en la Sección \ref{ssec:disenoFrontend}.

    \item \textbf{Capa de Aplicación y Lógica de Negocio (Servidor - \textit{Backend}):}
    ...
    \end{itemize}
    La organización interna y los patrones del \textit{backend} se explorarán en la Sección \ref{ssec:disenoBackend}.

    \item \textbf{Capa de Datos (Persistencia):}
    Esta capa es responsable del almacenamiento y la gestión de toda la información persistente de la aplicación. Para LogArt, se utiliza una base de datos NoSQL, concretamente \textbf{MongoDB}. En el contexto de la aplicación, esta capa estructura la información en las siguientes colecciones principales:
    \begin{itemize}
        \item \textbf{Colección de Usuarios (\texttt{users}):} Información de los usuarios registrados, incluyendo sus credenciales (contraseña hasheada mediante Bcrypt), correo electrónico, nombre de usuario y otra información de perfil. Además, incluye campos de control como el estado de verificación y el rol del usuario.
        \item \textbf{Colección de Objetos (\texttt{objects}):} Representan las entradas principales creadas por los usuarios. Cada documento es una obra (libro, videojuego o canción) y contiene metadatos específicos como el título, la descripción, la disciplina a la que pertenece y si se está compartiendo públicamente.
        \item \textbf{Colección de Comentarios (\texttt{comments}):} Los ``recuerdos'' o anotaciones que los usuarios asocian a las obras. Cada comentario está vinculado mediante referencias a un objeto específico y a un usuario creador, registrando marcas de tiempo de creación y modificación.
        \item \textbf{Colección de Disciplinas (\texttt{disciplines}):} Define las categorías principales bajo las que se clasifican los objetos de la aplicación.
        \item \textbf{Colección de Tokens Invalidados (\texttt{blacklistedtokens}):} Almacena de forma temporal los tokens JWT que han sido explícitamente invalidados (por ejemplo, tras un cierre de sesión seguro). Utiliza un índice TTL (\textit{Time-To-Live}) sobre el campo \texttt{expiresAt} para que MongoDB depure automáticamente los tokens expirados, optimizando el rendimiento.
    \end{itemize}
    
    Además de estructurar la información, esta capa proporciona los mecanismos subyacentes para realizar operaciones CRUD (Crear, Leer, Actualizar, Eliminar). Toda la interacción con la base de datos desde el \textit{backend} Node.js se realiza a través del ODM Mongoose, el cual facilita el modelado riguroso de estas colecciones y la validación de los datos antes de su persistencia final.
\end{enumerate}

El flujo de interacción típico comienza cuando el usuario realiza una acción en el \textit{frontend} de React. Este, a su vez, emite una petición HTTP a un endpoint específico de la API REST expuesta por el \textit{backend} en Express.js. El backend procesa la lógica de negocio asociada, interactúa con la base de datos MongoDB a través de Mongoose si la operación lo requiere, y devuelve una respuesta (generalmente en formato JSON) al \textit{frontend}, que actualiza la interfaz de usuario en consecuencia para reflejar el nuevo estado.

\begin{figure}[hbt!]
   \centering
  \begin{minipage}{0.95\textwidth}
   \centering
     \includegraphics[clip=true,width=\textwidth]{img/arquitectura_logica_logart_2.png}
  \end{minipage}
...
La arquitectura lógica descrita anteriormente se implementa y despliega utilizando tecnología de contenedorización, específicamente Docker y Docker Compose (ver Sección: \ref{sec:contenedores}). Este enfoque asegura la portabilidad, consistencia y facilidad de gestión del entorno de la aplicación, tanto para desarrollo como para la puesta en producción.
...
\begin{figure}
  ...
  \begin{minipage}{0.95\textwidth}
   \centering
     ...
     \includegraphics[clip=true,width=\textwidth]{img/arquitectura_despliegue_logart2.png}\\

  \end{minipage}
  \caption{Diagrama de Arquitectura de Despliegue Simplificado de LogArt.}\label{fig:arquitecturaDespliegue}
  \end{figure}

Esta arquitectura de empaquetado basada en contenedores facilita enormemente la reproducibilidad en el entorno de desarrollo local. Además, ha permitido realizar con éxito la transición a un entorno de producción real, garantizando que el contenedor que ejecuta el \textit{backend} y el \textit{frontend} mantenga el mismo comportamiento aislando las dependencias del sistema anfitrión, como se detalla en la Sección~\ref{ssec:despliegueProduccion}.

\subsection{Diseño del \textit{Backend}}
\label{ssec:disenoBackend}

El \textit{backend} de LogArt, desarrollado con Node.js y el framework Express.js, ha sido estructurado siguiendo principios de modularidad y separación de responsabilidades para facilitar su desarrollo, mantenimiento y comprensión. A continuación, se describe su organización interna y el modelo de datos subyacente.

\subsubsection{Modelo de Dominio}
\label{sssec:modeloDominioBackend}

El modelo de dominio del \textit{backend} representa las entidades principales de LogArt y sus interrelaciones. Estas entidades se corresponden directamente con los modelos definidos mediante Mongoose y las colecciones en la base de datos MongoDB. Las entidades clave son:
\begin{itemize}
    ...
    \item \textbf{Object (Objeto):} Representa una entrada de una obra creada por un usuario, asociada a una disciplina. Contiene atributos como título, descripción, imagen, fecha de creación, referencia al usuario propietario, y un indicador de si está compartido o no.
...
\begin{figure}
  ...
  \begin{minipage}{0.9\textwidth}
   \centering
     ...
     \includegraphics[clip=true,width=\textwidth]{img/diagrama_entidades_basedatos_3.png}\\
...
\begin{figure}
  ...
  \begin{minipage}{0.9\textwidth}
   \centering
     ...
     \includegraphics[clip=true,width=\textwidth]{img/diagrama_entidades_altonivel_2.png}\\
...
\begin{figure}
  ...
  \end{minipage}
  \caption{Diagrama de Módulos del \textit{Backend} de la Aplicación.}\label{fig:diagramaClasesBackend}
  \end{figure}


\subsection{Diseño del \textit{Frontend} (\textit{Single Page Application})}
\label{ssec:disenoFrontend}

El \textit{frontend} de LogArt se ha desarrollado como una \textit{Single Page Application} (SPA) utilizando la librería React. Este enfoque permite una experiencia de usuario fluida y dinámica, donde las transiciones entre diferentes vistas y la actualización de contenido se realizan sin necesidad de recargar la página completa desde el servidor. La estructura y los componentes principales del \textit{frontend} se describen a continuación.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{/ui-test/tests}:} Directorio que alberga las pruebas \textit{End-to-End} implementadas con Playwright para validar la interfaz de usuario y los flujos de interacción.
\end{itemize}
...
\begin{figure}
  ...
  \begin{minipage}{0.99\textwidth}
   \centering
     ...
     \includegraphics[clip=true,width=\textwidth]{img/diagrama_componentes_frontend_logart_2.png}\\

  \end{minipage}
  \caption{Diagrama de Componentes \textit{Frontend} de la aplicación.}\label{fig:diagramaComponentesFrontend}
  \end{figure}
...
Se explorarán tanto el diseño del \textit{backend}, centrándose en la estructura de su API RESTful y la integración con la base de datos, como el diseño \textit{frontend}, detallando la gestión del estado y la composición de la interfaz de usuario en React. Además, se abordarán aspectos específicos de la implementación, como la resolución de desafíos técnicos particulares o la descripción de una funcionalidad destacada del sistema. 
...
\subsection{Diseño e Implementación del \textit{Backend}}
\label{ssec:disenoImplBackend}

El \textit{backend} de LogArt, construido sobre Node.js y Express.js, es el responsable de la lógica de negocio, la gestión de datos y la exposición de una API RESTful. Su diseño se ha centrado en la modularidad, la separación de responsabilidades y la seguridad.

\subsubsection{API RESTful: Diseño y Flujo de Peticiones}
\label{sssec:apiRestfulDiseno}

La comunicación entre el \textit{frontend} y el \textit{backend} se realiza a través de una API RESTful. Se han seguido los principios estándar de diseño REST, como el uso de métodos HTTP semánticos (GET, POST, PUT, DELETE), la representación de recursos a través de URLs claras y el uso de códigos de estado HTTP para indicar el resultado de las operaciones. Todas las respuestas de la API se devuelven en formato JSON.
...
\begin{enumerate}
    \item El cliente (\textit{frontend}), a través de alguna acción en la interfaz, realiza una petición HTTP a un endpoint de la API, incluyendo un JSON Web Token (JWT) en la cabecera \texttt{Authorization} (ej. \texttt{Bearer token}).
...
\begin{itemize}
    \item La ruta \texttt{POST /api/v1/objects} está protegida por \texttt{verifyToken} y utiliza el middleware \verb|uploadObject.single('imageUrl')| para manejar la subida de la imagen.
...
En resumen, el diseño del \textit{backend} de LogArt se ha organizado de forma modular para garantizar la separación de responsabilidades entre rutas, controladores, servicios y repositorios. Gracias al uso de middleware para autenticación, la lógica de validación y la de gestión de errores, así como a la abstracción de la persistencia mediante Mongoose, conseguimos un API RESTful robusto, seguro y fácil de mantener. Además, la integración con Swagger facilita la generación ``Automática'' de la documentación.

A continuación, en la Sección \ref{ssec:disenoImplFrontend}, abordaremos el diseño e implementación del \textit{frontend} en React, donde veremos cómo esta capa consume la API, maneja el estado de la aplicación y construye la interfaz de usuario en LogArt.

\subsection{Diseño e Implementación del \textit{Frontend} (React)}
\label{ssec:disenoImplFrontend}

El \textit{frontend} de LogArt, desarrollado como un \textit{Single Page Application} (SPA) con React, es el punto de interacción directo con el usuario. Su diseño se ha enfocado en la creación de una interfaz intuitiva, responsiva y modular, utilizando componentes reutilizables y una gestión de estado eficiente.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Proporciona funciones para \texttt{login}, \texttt{register}, \texttt{logout}, que encapsulan las llamadas a la API del \textit{backend} y actualizan el estado de autenticación en consecuencia. La función \texttt{logout} también se encarga de eliminar el token de \texttt{localStorage} y de la lista negra del \textit{backend}.
    \end{itemize}
...
La interacción con la API RESTful del \textit{backend} se centraliza principalmente a través de una instancia de Axios configurada en \texttt{utilities/api.jsx}. Esta instancia define la \texttt{baseURL} del servidor \textit{backend} (\texttt{https://localhost:8443}) y utiliza interceptores de petición para adjuntar automáticamente el token JWT (obtenido de \texttt{localStorage} a través del \texttt{AuthContext}) a la cabecera \texttt{Authorization} de cada solicitud, simplificando las llamadas a endpoints protegidos.
...
\begin{itemize}
    ...
    \item Manejar la lógica de paginación, búsqueda (implementando \textit{debounce} o antirrebote, como se puede ver en \texttt{ObjectsByDiscipline.jsx}, para optimizar las peticiones mientras el usuario escribe) y filtrado (ej. ''mostrar solo favoritos'').
    \item Pasar los datos y las funciones de \textit{callback} necesarias a los componentes de presentación hijos (componentes visuales). 
    \item Actualizar su estado local (y, por ende, la interfaz de usuario o UI) en respuesta a las acciones del usuario o a los datos recibidos de la API.
\end{itemize}
El componente \texttt{App.jsx} actúa como el orquestador principal del \textit{frontend}, configurando el enrutador \texttt{react-router-dom} para mapear las URLs a los componentes de página correspondientes. Utiliza el componente \texttt{ProtectedRoute} para restringir el acceso a ciertas rutas basándose en el estado de autenticación del usuario y, opcionalmente, en su rol (ej. para el \texttt{/admin/dashboard}).

\subsubsection{Componentes Reutilizables Clave}
\label{sssec:frontendComponentesReutilizables}

La modularidad del \textit{frontend} se logra mediante el uso extensivo de componentes reutilizables, algunos ejemplos destacados son:
\begin{itemize}
...
\subsection{El Panel de Administración}
\label{ssec:focoAdminDashboard}

\begin{figure}[hbt!]
   \centering
   \centering
    \includegraphics[width=0.95\textwidth]{img/dashboard2.png}

  \caption{Pantalla de \textit{Dashboard} de Administrador.}\label{fig:pantallaDashboard}
  \end{figure}

Una de las funcionalidades avanzadas y más complejas implementadas en LogArt es el Panel de Administración o Admin \textit{Dashboard}. Esta sección está diseñada para proveer al rol de Administrador una visión general del estado de la plataforma, estadísticas detalladas sobre usuarios y contenido, y herramientas para la moderación. A continuación, se detalla su diseño e implementación tanto en el \textit{backend} como en el \textit{frontend}.
\paragraph{Requisitos Específicos del Admin \textit{Dashboard}:}
El \textit{dashboard} debía cumplir con los siguientes requisitos funcionales principales (derivados de RF6 en la Sección \ref{rf:adminFuncionalidades}):
\begin{itemize}
    \item Presentar un resumen general (\textit{overview}) con métricas clave (total de usuarios, objetos, comentarios).
...
\paragraph{Diseño e Implementación en el \textit{Backend}:}El backend expone una serie de endpoints específicos bajo la ruta \texttt{/api/v1/dashboard/}, todos protegidos para ser accesibles únicamente por administradores mediante el middleware \texttt{authMiddleware.js}
\begin{itemize}
    \item \textbf{Rutas y Controladores:} El archivo \texttt{dashboardRoutes.js} define las rutas para cada sección del \textit{dashboard} (ej. \texttt{/overview} , \texttt{/activity, etc}). Cada ruta está asociada a una función en \texttt{dashboardController.js}, que actúa como intermediario, recibiendo la petición y llamando al servicio correspondiente.
    \item \textbf{Servicio del \textit{Dashboard} (\texttt{dashboardService.js}):} Este servicio es el corazón de la lógica del \textit{dashboard} en el \textit{backend}. Contiene funciones que realizan agregaciones complejas y consultas a la base de datos MongoDB utilizando los modelos Mongoose (a través de los repositorios) para recopilar y procesar los datos necesario para cada estadística. Destacan las siguientes funcionalidades:
    \begin{itemize}
...
\paragraph{Diseño e Implementación en el \textit{Frontend} (\textbf{AdminDashboard.jsx} y componentes hijos):}

El \textit{frontend} del Admin \textit{Dashboard}, implementado como un componente React Principal (\texttt{AdminDashboard.jsx}), se encarga de presentar la información de forma organizada y permitir la interacción del administrador.
\begin{itemize}
    \item \textbf{Estructura de Pestañas (Tabs):} La interfaz del \textit{dashboard} se organiza en varías pestañas (Resumen, Usuarios, Contenido, Actividad, Crecimiento, Objetos), cada una correspondiente a una sección de datos. El \texttt{AdminDashboard.jsx} se gestiona el estado de la pestaña activa (\texttt{activeTab}) y renderiza el contenido correspondiente. La navegación entre pestañas se actualiza utilizando \texttt{useEffect} para observar cambios en \texttt{location.pathname}.
    \item \textbf{Obtención de Datos:} Al cambiar de pestaña o de periodo (para Actividad y Crecimiento), se realiza una petición a la API del \textit{backend} (usando la instancia de Axios configurada) para obtener los datos específicos de esa sección. Se manejan estados de carga (\texttt{loading}) y error (\texttt{error}) para informar al usuario.
    \item \textbf{Componentización de Vistas:} Cada pestaña tiene componentes dedicados para renderizar sus datos (\texttt{OverviewTab}, \texttt{UsersTab}, \texttt{GrowthTab} entre otros). Estos componentes reciben los datos obtenidos del \textit{backend} como props y los presentan utilizando elementos de UI como tarjetas de estadísticas (\texttt{StatCard}), tablas y gráficos.
    \item \textbf{Interactividad y Moderación:} 
    \begin{itemize}
        \item En la pestaña ``Usuarios'' (\texttt{UsersTab}), se muestra una lista paginada de usuarios y se ofrece la opción de eliminar usuarios (excepto otros administradores). La eliminación de un usuario desencadena una petición DELETE a la API (junto con sus recursos asociados) y actualiza la UI localmente y las estadísticas del \textit{dashboard}.
...
Este desarrollo integrado entre el \textit{backend} (con sus servicios de agregación de datos) y el \textit{frontend} (con su estructura de componentes y gestión de estado) permite ofrecer una herramienta de administración potente y útil para la supervisión de LogArt.
...
\begin{itemize}
    \item \textbf{Pruebas del \textit{Backend} (API RESTful):}
    \begin{itemize}
        \item Se desarrollaron pruebas de integración y unitarias para el \textit{backend} utilizando el framework \textbf{Jest} y la librería \textbf{Supertest}. Estas pruebas se enfocaron en validar la lógica de los controladores, los servicios y la interacción con la base de datos (a través de los repositorios y modelos Mongoose)
    ...
    \end{itemize}
    \vspace{0.3cm}
    \item \textbf{Pruebas \textit{End-To-End} del \textit{Frontend}:}
    \begin{itemize}
        \item Para el \textit{frontend} React, se implementaron pruebas E2E con \textbf{Playwright}. Estas pruebas automatizan la interacción con la interfaz de usuario en un navegador (Chromium), validando flujos completos desde la perspectiva del usuario, como el registro, inicio de sesión, creación de objetos y comentarios, la navegación, y la edición de perfil.
    \end{itemize}
    \vspace{0.3cm}
    \item \textbf{Integración con el Pipeline de CI/CD:}
    \begin{itemize}
        \item Como se describió en la Sección \ref{sec:cicd}, tanto las pruebas del \textit{backend} como las pruebas E2E del \textit{frontend} se integran en el workflow de Integración Continua de GitHub Actions. Se ejecutan automáticamente ante cada \textit{Pull Request} a la rama \texttt{main}, proporcionando una capa de seguridad esencial antes de integrar nuevos cambios.
    \end{itemize}
\end{itemize}

\paragraph{Cobertura de Código del \textit{Backend}:}
Las pruebas automatizadas del \textit{backend}, gestionadas con Jest, proporcionan una medida cuantitativa de la exhaustividad de las pruebas. Según el informe de cobertura generado (ver Figura \ref{fig:coberturaBackend}), se alcanzó una cobertura global del:
\begin{itemize}
...
\begin{figure}
  ...
  \end{minipage}
  \caption{Informe de Cobertura de Pruebas del \textit{Backend} (Jest).}\label{fig:coberturaBackend}
  \end{figure}
...
La capacidad de empaquetar la aplicación de forma consistente y desplegarla en diferentes entornos es crucial. LogArt utiliza Docker para la contenedorización y GitHub Actions para automatizar la creación y publicación de imágenes, facilitando tanto el desarrollo local como la puesta en producción.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Este \texttt{Dockerfile} primero construye el \textit{frontend} React en una etapa llamada (\textbf{frontend-build}, luego compila el \textit{backend} Node.js en otra (\textbf{backend-build}), optimizando las dependencias y eliminando las de desarrollo.
        \item Finalmente, una etapa de producción basada en \texttt{node:20-alpine} copia los artefactos compilados del \textit{backend} y el \textit{frontend} (este último a una carpeta \texttt{/public} dentro del \textit{backend}), resultando en una imagen final ligera y optimizada que contiene la aplicación completa lista para ejecutarse. La aplicación se configura para ejecutarse en el puerto 8443 en modo producción.
    \end{itemize}
    \vspace{0.3cm}
    ...
    \begin{itemize}
        ...
        \item La imagen construida se etiqueta con múltiples tags (versión semántica, \texttt{latest} para releases estables, y hash del commit) y se publica en un registro público de contenedores, \textbf{DockerHub} (bajo el repositorio ``davidmorenoo/logartapp''\footnote{\url{https://hub.docker.com/r/davidmorenoo/logartapp}}.
        ...
        \end{itemize}
        \vspace{0.3cm}
        \item \textbf{Despliegue en Producción Real:}
    \label{ssec:despliegueProduccion}
    Para validar la viabilidad y robustez del proyecto fuera del entorno de desarrollo local, se ha llevado a cabo el despliegue completo de LogArt en un entorno de producción accesible públicamente a través de Internet. Este hito se ha logrado apoyándose en la arquitectura de contenedores previamente descrita y empleando soluciones PaaS (\textit{Platform as a Service}) orientadas a despliegues ágiles:
        \begin{itemize}
            \item \textbf{Infraestructura y Plataforma:} La ejecución de la aplicación (el contenedor que unifica el \textit{backend} de Node.js y los archivos estáticos de React) se ha delegado a \textbf{Railway\footnote{\url{https://railway.com}}}. Esta plataforma lee de forma nativa el \texttt{Dockerfile} alojado en el repositorio, construye la imagen y orquesta el contenedor.
            \item \textbf{Gestión de la Base de Datos:} Siguiendo las buenas prácticas de la industria, la persistencia de datos no se ha desplegado en un contenedor efímero dentro de Railway, sino que se ha aprovisionado como un servicio gestionado para garantizar su fiabilidad y rendimiento.
            \item \textbf{Conectividad y Seguridad:} La comunicación entre el servicio web y la base de datos se realiza mediante cadenas de conexión protegidas inyectadas como variables de entorno en el panel de Railway. Asimismo, la plataforma se encarga de proveer y gestionar automáticamente los certificados SSL válidos, habilitando tráfico HTTPS sin necesidad de configuración manual de claves criptográficas en el contenedor.
            \item \textbf{Ahorro de Recursos:} Aprovechando las capacidades de \textit{cloud computing} moderno y para optimizar los costes, la instancia en Railway permite ser pausada cuando no se encuentra en uso, pudiendo reactivarse en cuestión de segundos para demostraciones o sesiones de evaluación, conservando la integridad de los datos.
        \end{itemize}
\end{itemize}

Este enfoque de distribución, respaldado por las prácticas de CI/CD, ha permitido materializar la solución y hacer que LogArt sea funcional, seguro y accesible públicamente en una URL de producción\footnote{\url{https://2024-logart-deploy-demo-production.up.railway.app}}

\begin{figure}[hbt!]
   \centering
   \includegraphics[width=0.95\textwidth]{img/logart_produccion.png}
   \caption*{(a) Vista de la interfaz de usuario de LogArt operando en la URL pública.}
   
   \vspace{0.8cm}
   \includegraphics[width=0.95\textwidth]{img/panel_railway.png}
   \caption*{(b) Vista del panel de servicios y orquestación en Railway.}
   
   \vspace{0.4cm}
   \caption{Detalles del despliegue de LogArt en la nube: aplicación y panel de control.}
   \label{fig:despliegue_completo}
\end{figure}