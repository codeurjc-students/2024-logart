...
\begin{itemize}
    \item \textbf{¿Qué es?:} Node.js\footnote{\url{https://nodejs.org/}} es un entorno de ejecución de JavaScript del lado del servidor, asíncrono y orientado a eventos, construido sobre el motor V8 de JavaScript de Google Chrome. Permite construir aplicaciones escalables con gran rendimiento \cite{kinstaQueEsNodeJS}.
...
\begin{itemize}
    \item \textbf{JSON Web Tokens (JWT}
        \begin{itemize}
            \item \textbf{¿Qué es?:} JSON Web Token (JWT)\footnote{\url{https://datatracker.ietf.org/doc/html/rfc7519}} es un formato estandarizado por el RFC 7519 que permite intercambiar información de forma compacta entre dos partes mediante un objeto JSON. Al ir firmado digitalmente, el contenido puede verificarse y confiar en que no ha sido modificado. Esta firma puede generarse con una clave secreta mediante HMAC o con un esquema de clave pública y privada, como RSA o ECDSA. \cite{caronteJWT}.
            \item \textbf{¿Por qué se eligió para LogArt?:}
                \begin{itemize}
...
La elección de un Entorno de Desarrollo Integrado (IDE) o editor de código adecuado es crucial para la productividad del desarrollador. Para el desarrollo de LogArt, se utilizó:
    \begin{itemize}
        \item \textbf{Visual Studio Code (VS Code)}
            \begin{itemize}
                \item \textbf{¿Qué es?:} VS Code\footnote{\url{https://code.visualstudio.com/}} es un editor de código creado por Microsoft que destaca por combinar un funcionamiento ágil con herramientas avanzadas de desarrollo. Entre sus funciones principales se encuentran el coloreado de sintaxis, sugerencias inteligentes de código, depuración incorporada, compatibilidad con Git y una amplia colección de extensiones que permiten adaptar el entorno a las necesidades del desarrollador.
...
La contenedorización consiste en encapsular una aplicación junto con todos los recursos y dependencias que necesita dentro de un entorno aislado y portable. De este modo, el software puede desarrollarse, distribuirse y ejecutarse de forma más consistente en distintos sistemas. Para LogArt, se adoptaron las siguientes herramientas:
