...
describe('Pruebas de Usuarios', () => {
  ...
  beforeAll(async () => {
    ...
    const userLogin = await authService.login('usuario@example.com', 'password123');
    ...
    const { accessToken} = await authService.login('admin@example.com', 'adminpass123');
  ...
  describe('DELETE /api/v1/users/:userId', () => {
    ...
    role: 'admin'
      ...
      expect(response.statusCode).toBe(401);
      expect(response.body).toHaveProperty('error', true);
      expect(response.body).toHaveProperty('message', 'You cannot delete your own account, contact the system administrator');
    });
